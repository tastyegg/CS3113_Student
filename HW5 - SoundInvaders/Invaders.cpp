...
void Invaders::Collision(Entity *bullet) {
	for (int j = 0; j < 5; j++) {
		for (int i = 0; i < 11; i++) {
			if (pawns[i][j]) {
				if (!((bullet->y - bullet->height / 2) > (height + spacing/2) * (5 + y - j) + height ||
					(bullet->y + bullet->height / 2) < (height + spacing/2) * (5 + y - j) ||
...
bool Invaders::TankCollision(Tank *tank) {
	for (int b = 0; b < 4; b++) {
		if (bullets[b]->use) {
			if (!((bullets[b]->y - bullets[b]->height / 2) > tank->y + tank->height / 2 ||
				(bullets[b]->y + bullets[b]->height / 2) < tank->y - tank->height / 2 ||
				...
				for (int j = 0; j < 5; j++) {
					if (pawns[i][j]) {
						...
						if (randPawn < 0) {
							...
							bullets[b]->y = (height + spacing/2) * (5 + y - j) + height / 2;
							i = 11; j = 5;
						}
					}
				}
...
void Invaders::Draw() {
	...
	modelMatrix.Translate(x - fmod(x, width/ (8 * speed)) - width / 2 - 5 * (width + spacing),
		(height + spacing/2) * (5 + y) + height / 2, 0.0f);
	...
	*/
	...
	for (int j = 0; j < 5; j++) {
		...
		modelMatrix.Translate((width + spacing) * -11, -height - spacing/2, 0);
	}
