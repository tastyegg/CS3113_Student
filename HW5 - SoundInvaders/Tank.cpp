...
void Tank::DrawSpriteSheetSprite(int index, int spriteCountX, int spriteCountY) {
	...
	float vertices[] = { -width * 5 / 2, -height * 5 / 2 * (36.0f / 52.0f), width * 5 / 2, height * 5 / 2 * (36.0f / 52.0f), -width * 5 / 2, height * 5 / 2 * (36.0f / 52.0f),
		width * 5 / 2, height * 5 / 2 * (36.0f / 52.0f), -width * 5 / 2, -height * 5 / 2 * (36.0f / 52.0f), width * 5 / 2, -height * 5 / 2 * (36.0f / 52.0f) };
...
void Tank::CreateBullet() {
	if (!bullet->use) {
		...
		bullet->x = x;
		bullet->y = y;
		rotation = (float)(((rand() % 2) * 2) - 1);
	}
}

void Tank::Update(float elapsed) {
	rotation += elapsed * 32 * ((rotation > 0) * 2 - 1);
	if (bullet->use) {
